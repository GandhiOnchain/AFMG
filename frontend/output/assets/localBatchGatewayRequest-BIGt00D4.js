import { ht as __vitePreload } from "./index-CESBTgrc.js";
...
var BaseError$2 = class BaseError$2 extends Error {
	constructor(shortMessage, args = {}) {
		...
		const docsPath$5 = args.cause instanceof BaseError$2 ? args.cause.docsPath || args.docsPath : args.docsPath;
		const message = [
			shortMessage || "An error occurred.",
			"",
			...args.metaMessages ? [...args.metaMessages, ""] : [],
			...docsPath$5 ? [`Docs: https://abitype.dev${docsPath$5}`] : [],
...
function hexToBigInt(hex, opts = {}) {
	...
	const max = (1n << BigInt(size$3) * 8n - 1n) - 1n;
...
function encodeArray(value, { length, param }) {
	...
	if (dynamic || dynamicChild) {
		...
		if (dynamic) {
			...
			return {
				dynamic: true,
				encoded: preparedParams.length > 0 ? concat$1([length$1, data]) : length$1
			};
		}
...
function decodeArray(cursor, param, { length, staticPosition }) {
	if (!length) {
		...
		const length$1 = bytesToNumber(cursor.readBytes(32));
...
function extract(value_, { format }) {
	if (!format) return {};
	const value = {};
	function extract_(formatted$1) {
		const keys = Object.keys(formatted$1);
		for (const key of keys) {
			if (key in value_) value[key] = value_[key];
			if (formatted$1[key] && typeof formatted$1[key] === "object" && !Array.isArray(formatted$1[key])) extract_(formatted$1[key]);
		}
	}
...
var BaseError$1 = class BaseError$1 extends Error {
	constructor(shortMessage, options = {}) {
		...
		const docs = `https://oxlib.sh${docsPath$5 ?? ""}`;
		const message = [
			shortMessage || "An error occurred.",
...
function fromNumber(value, options = {}) {
	...
	const hex = `0x${(signed && value_ < 0 ? (1n << BigInt(size$3 * 8)) + BigInt(value_) : value_).toString(16)}`;
...
async function call(client, args) {
	...
	try {
		...
		const { offchainLookup, offchainLookupSignature } = await __vitePreload(async () => {
			const { offchainLookup: offchainLookup$1, offchainLookupSignature: offchainLookupSignature$1 } = await import("./ccip-DkYQr3mW.js");
			return {
				offchainLookup: offchainLookup$1,
				offchainLookupSignature: offchainLookupSignature$1
			};
		}, []);
		if (client.ccipRead !== false && data$1?.slice(0, 10) === offchainLookupSignature && to) return { data: await offchainLookup(client, {
			data: data$1,
			to
		}) };
...
export { getNodeError as $, parseAbiParameter as $n, arrayRegex as $t, padRight as A, toHex as An, CallExecutionError as At, toBigInt$1 as B, AbiEventNotFoundError as Bn, prettyPrint as Bt, concat as C, hexToBytes as Cn, UnsupportedChainIdError as Ct, fromNumber as D, bytesToHex as Dn, HttpRequestError as Dt, fromBytes as E, boolToHex as En, UserRejectedRequestError as Et, from$1 as F, trim$1 as Fn, getUrl as Ft, stringify as G, DecodeLogTopicsMismatch as Gn, decodeErrorResult as Gt, toNumber$1 as H, AbiEventSignatureNotFoundError as Hn, formatEther as Ht, fromHex as I, pad$2 as In, InvalidSerializableTransactionError as It, isAddressEqual as J, size$2 as Jn, PositionOutOfBoundsError as Jt, BaseError$1 as K, UnsupportedPackedAbiType as Kn, decodeAbiParameters as Kt, fromString$1 as L, AbiDecodingDataSizeTooSmallError as Ln, TransactionNotFoundError as Lt, slice$1 as M, hexToBool as Mn, ContractFunctionRevertedError as Mt, toNumber as N, hexToNumber as Nn, ContractFunctionZeroDataError as Nt, fromString as O, numberToHex as On, RpcRequestError as Ot, validate as P, hexToString as Pn, RawContractError as Pt, extract as Q, parseStructs as Qn, encodeAbiParameters as Qt, size$1 as R, AbiDecodingZeroDataError as Rn, TransactionReceiptNotFoundError as Rt, IntegerOutOfRangeError as S, toBytes$1 as Sn, UnknownRpcError as St, fromBoolean as T, toBytes as Tn, UnsupportedProviderMethodError as Tt, toString as U, BytesSizeMismatchError as Un, formatUnits as Ut, toBoolean as V, AbiEventSignatureEmptyTopicsError as Vn, formatGwei as Vt, trimLeft as W, DecodeLogDataMismatch as Wn, stringify$1 as Wt, serializeStateOverride as X, formatAbiItem as Xn, parseAccount as Xt, assertRequest as Y, isHex as Yn, encodeFunctionData as Yt, formatTransactionRequest as Z, parseAbi as Zn, getAbiItem as Zt, multicall3Abi as _, aoutput as _n, ResourceUnavailableRpcError as _t, createBatchScheduler as a, concatHex as an, modifiers as ar, DuplicateIdError as at, universalResolverReverseAbi as b, createView as bn, UnauthorizedProviderError as bt, getChainContractAddress as c, getAddress as cn, InvalidParamsRpcError as ct, erc6492SignatureValidatorByteCode as d, toEventSelector as dn, LimitExceededRpcError as dt, bytesRegex as en, parseSignature as er, UnknownNodeError as et, multicall3Bytecode as f, keccak256 as fn, MethodNotFoundRpcError as ft, erc6492SignatureValidatorAbi as g, aexists as gn, ResourceNotFoundRpcError as gt, erc20Abi as h, abytes as hn, ProviderDisconnectedError as ht, call as i, concat$1 as in, isStructSignature as ir, ChainDisconnectedError as it, size as j, hexToBigInt as jn, ContractFunctionExecutionError as jt, padLeft as k, stringToHex as kn, TimeoutError as kt, encodeDeployData as l, LruMap as ln, InvalidRequestRpcError as lt, erc1271Abi as m, Hash as mn, ParseRpcError as mt, localBatchGatewayUrl as n, slice as nn, InvalidAbiParametersError as nr, AtomicityNotSupportedError as nt, withResolvers as o, isAddress as on, formatAbiItem$1 as or, InternalRpcError as ot, addressResolverAbi as p, keccak_256 as pn, MethodNotSupportedRpcError as pt, decodeFunctionResult as q, BaseError as qn, createCursor as qt, decodeFunctionData as r, sliceHex as rn, InvalidAbiItemError as rr, BundleTooLargeError as rt, getCallError as s, checksumAddress as sn, formatAbiParameters as sr, InvalidInputRpcError as st, localBatchGatewayRequest as t, integerRegex as tn, splitParameters as tr, AtomicReadyWalletRejectedUpgradeError as tt, deploylessCallViaBytecodeBytecode as u, InvalidAddressError as un, JsonRpcVersionUnsupportedError as ut, textResolverAbi as v, clean as vn, SwitchChainError as vt, from as w, stringToBytes as wn, UnsupportedNonOptionalCapabilityError as wt, toRpc as x, rotr as xn, UnknownBundleIdError as xt, universalResolverResolveAbi as y, createHasher as yn, TransactionRejectedRpcError as yt, slice$2 as z, AbiEncodingLengthMismatchError as zn, WaitForTransactionReceiptTimeoutError as zt };
