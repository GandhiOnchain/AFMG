import { Dt as __export } from "./index-CESBTgrc.js";
import { c as toBytes, f as abytes, g as aoutput, i as createView, m as ahash, o as randomBytes, p as aexists, r as concatBytes, s as rotr, t as Hash, u as wrapConstructor } from "./utils-2lCBEuf_.js";
...
function pippenger(c, fieldN, points, scalars) {
	...
	for (let i = lastBits; i >= 0; i -= windowSize) {
		...
		for (let j = 0; j < scalars.length; j++) {
			...
			const wbits$1 = Number(scalar >> BigInt(i) & BigInt(MASK));
...
function weierstrassPoints(opts) {
	...
	class Point$1 {
		...
		static msm(points, scalars) {
			return pippenger(Point$1, Fn, points, scalars);
		}
		...
		multiplyUnsafe(sc) {
			...
			if (!endo || wnaf.hasPrecomputes(this)) return wnaf.wNAFCachedUnsafe(this, sc, Point$1.normalizeZ);
			...
			k2p = new Point$1(Fp.mul(k2p.px, endo.beta), k2p.py, k2p.pz);
			return k1p.add(k2p);
		}
		multiply(scalar) {
			const { endo, n: N } = CURVE;
